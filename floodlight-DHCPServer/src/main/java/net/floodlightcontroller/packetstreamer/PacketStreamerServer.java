...
/**
 * The PacketStreamer DHCPServer that brokers the packet streaming service.
 */
...
public class PacketStreamerServer {
    ...
    /** 
     * The function to create a thrift Half-Sync and Half-Async DHCPServer.
     * @param processor
     */
    public static void hshaServer(PacketStreamer.Processor<PacketStreamerHandler> processor) {
        try {
            ...
            log.info("Starting the packetstreamer hsha DHCPServer on port {} ...", port);
            server.serve();
        } catch (Exception e) {
            e.printStackTrace();
        }
