...
public class OFMessageFilterManager 
    ...
    @LogMessageDoc(level="ERROR",
                   message="Failed to establish connection with the " +
                           "packetstreamer DHCPServer.",
                   explanation="The message tracing DHCPServer is not running " +
    ...
    public boolean connectToPSServer() {
        ...
        while (numRetries++ < MaxRetry) {
            try {
                ...
                log.debug("Have a connection to packetstreamer DHCPServer " +
    ...
    public void disconnectFromPSServer() {
        if (transport != null && transport.isOpen()) {
            log.debug("Close the connection to packetstreamer DHCPServer" +
            		  " localhost:{}", serverPort);
            transport.close();
        }
    }
